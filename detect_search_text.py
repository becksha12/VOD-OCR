...
if __name__ == '__main__':
    ...
    for image in video_utils.get_frame_for_every_second(VIDEO_PATH):
        ...
        if is_keyboard_present(gray, template, template_w, template_h):
            predictions = search_bar_text(image, model)
            video_ocr.display_bounding_boxes(predictions, image, save_path=f'{OUTPUT_DIR}/frame_{counter:04d}.png')
